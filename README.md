...
/* Limpa o resto da linha no stdin (consome até '\n' ou EOF) */
void limpa_buffer() {
    int c;
    while ((c = getchar()) != '\n' && c != EOF) { }
}
...
Elemento* insere_contato(Elemento *lista) {
    ...
    printf("Nome: ");
    /* usamos fgets e garantimos que não haja lixo no buffer */
...
int main() {
    setlocale(LC_ALL, ""); /* ativa locale para acentos (se o terminal for UTF-8) */
